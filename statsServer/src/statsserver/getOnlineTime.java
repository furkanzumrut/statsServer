...
public static void main(String args[]) {
    getOnlineTime ini = new getOnlineTime();
    ini.doit();
           ...
           if (listOfFiles[i].isFile()) 
           {
            files = listOfFiles[i].getName();
             if (files.endsWith(".dudb.sav"))
             {
                ...
                if(!p.getProperty("onlineTime1").isEmpty())
                userMap.put(Integer.parseInt(p.getProperty("onlineTime1")), decode(nickname));
                //System.out.println(nickname+" "+p.getProperty("onlineTime1"));
